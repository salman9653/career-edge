  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      batch.update(notificationRef, {
        message: `${
          (notificationData.applicantCount || 1) + 1
        } new candidates have applied for ${jobData.title}.`,
  ...
  try {
    ...
    if (currentRound.type === "assessment" && currentRound.assessmentId) {
      const assessmentDoc = await getDoc(
        doc(db, "assessments", currentRound.assessmentId)
      );
      if (assessmentDoc.exists()) {
        ...
        if (currentRound.selectionCriteria) {
          resultStatus =
            score >= currentRound.selectionCriteria ? "Passed" : "Failed";
        }
      }
    }
...
export async function submitAssessmentFeedbackAction(
  ...
  try {
    ...
    const roundResultIndex = roundResults.findIndex(
      (r) => r.roundId === roundId
    );
...
export async function fetchCandidateDashboardData(userId: string) {
  try {
    return await fetchCandidateDashboardDataServer(userId);
  } catch (error) {
