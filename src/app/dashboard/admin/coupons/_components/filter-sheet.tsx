...
import type { FilterState } from './coupons-table';
...
const typeOptions = ['coupon', 'offer'];
...
export function FilterSheet({ filters, onFilterChange }: FilterSheetProps) {
  ...
  const clearFilters = () => {
    const clearedFilters = { status: [], discountType: [], type: [] };
  ...
  const activeFilterCount =
    filters.status.length + filters.discountType.length + filters.type.length;
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-[300px] sm:w-[400px] flex flex-col">
        ...
        <div className="flex-1 space-y-6 py-6">
          ...
          <div>
            <h3 className="font-semibold mb-4">Discount Type</h3>
            <div className="space-y-3">
              {discountTypeOptions.map((type) => (
                <div key={type} className="flex items-center space-x-2">
                  <Checkbox
                    id={`type-${type}`}
                    checked={localFilters.discountType.includes(type)}
                    onCheckedChange={(checked) =>
                      handleCheckedChange('discountType', type, !!checked)
                    }
                  />
                  <Label
                    htmlFor={`type-${type}`}
