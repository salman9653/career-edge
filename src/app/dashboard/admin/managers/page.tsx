...
export default function AdminManagersPage() {
  ...
  useEffect(() => {
    if (session?.role === 'admin') {
        const managersQuery = query(collection(db, 'users'), where('role', '==', 'adminAccountManager'));
    ...
    <div className="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
      ...
      <div className="flex flex-col max-h-screen">
        <header className="flex h-16 shrink-0 items-center justify-between gap-4 border-b bg-background px-4 md:px-6 sticky top-0 z-30 md:static">
            <h1 className="font-headline text-xl font-semibold md:ml-0 ml-12">Platform Account Managers</h1>
            <MobileSearch />
        </header>
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>Manage other platform administrators.</CardDescription>
                        </div>
