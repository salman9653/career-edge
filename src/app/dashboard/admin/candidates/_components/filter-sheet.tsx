...
import type { FilterState } from './candidates-table';
...
const subscriptionOptions = ['Free', 'Pro', 'Pro+'];
...
export function FilterSheet({ filters, onFilterChange }: FilterSheetProps) {
  ...
  const clearFilters = () => {
    const clearedFilters = { status: [], subscription: [] };
  ...
  const activeFilterCount =
    filters.status.length + filters.subscription.length;
            ...
            <div className="space-y-3">
              {subscriptionOptions.map((plan) => (
                <div key={plan} className="flex items-center space-x-2">
                  <Checkbox
                    id={`plan-${plan}`}
                    checked={localFilters.subscription.includes(plan)}
                    onCheckedChange={(checked) =>
                      handleCheckedChange('subscription', plan, !!checked)
                    }
                  />
                  <Label
                    htmlFor={`plan-${plan}`}
