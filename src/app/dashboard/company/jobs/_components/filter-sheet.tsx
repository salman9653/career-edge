...
import type { FilterState } from './jobs-table';
...
export function FilterSheet({ filters, onFilterChange, uniqueLocations, uniqueJobTypes }: FilterSheetProps) {
  ...
  const clearFilters = () => {
    const clearedFilters = { status: [], jobType: [], location: [] };
    setLocalFilters(clearedFilters);
    onFilterChange(clearedFilters);
    setIsOpen(false);
  };

  const activeFilterCount = filters.status.length + filters.jobType.length + filters.location.length;
            ...
            <div className="space-y-3">
              {statusOptions.map((status) => (
                <div key={status} className="flex items-center space-x-2">
                  ...
                  />
                  <Label htmlFor={`status-${status}`} className="w-full font-normal capitalize">
                    {status}
                  </Label>
                </div>
              ))}
              ...
              {uniqueJobTypes.map((type) => (
                <div key={type} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleCheckedChange('jobType', type, !!checked)
                    }
                  />
              ...
              {uniqueLocations.map((location) => (
                <div key={location} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleCheckedChange('location', location, !!checked)
                    }
                  />
