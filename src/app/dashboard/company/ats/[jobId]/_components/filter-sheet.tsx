...
import type { FilterState } from './applicants-table';
...
export function FilterSheet({ filters, onFilterChange, stageOptions }: FilterSheetProps) {
            ...
            <div className="space-y-3">
              {stageOptions.map((stage) => (
                <div key={stage} className="flex items-center space-x-2">
                  <Checkbox
                    id={`status-${stage}`}
                    checked={localFilters.status.includes(stage)}
                    onCheckedChange={(checked) =>
                      handleCheckedChange('status', stage, !!checked)
                    }
                  />
                  <Label
                    htmlFor={`status-${stage}`}
