...
import AssessmentStartContent from './_components/assessment-content';
...
export default async function AssessmentStartPage({ params }: AssessmentStartPageProps) {
    const { jobId, roundId } = await params;
    
    return (
        <Suspense fallback={<AssessmentPageLoading />}>
            <AssessmentStartContent jobId={jobId} roundId={roundId} />
        </Suspense>
    );
}
